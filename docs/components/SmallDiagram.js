  ...
  var counters = Object.keys(state).map(function(id) {
    ...
    return (
      <g key={id}>
        ...
        <text x={cell.cx} y={cell.cy + 5} textAnchor="middle" fill="white" fontSize="14" fontWeight="bold">
          {st === 'red' ? '1' : '0'}
        </text>
      </g>
    );
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '2px solid var(--line)', borderRadius: '12px', padding: '8px', display: 'flex', justifyContent: 'center' }}>
      <svg width="280" height="280" viewBox="0 0 250 250">
        ...
        <text x="8" y="17" fill="var(--sea-ink-soft)" fontSize="15" fontWeight="bold">5</text>
        <text x="242" y="17" textAnchor="end" fill="var(--sea-ink-soft)" fontSize="15" fontWeight="bold">6</text>
        <text x="8" y="243" fill="var(--sea-ink-soft)" fontSize="15" fontWeight="bold">7</text>
        <text x="242" y="243" textAnchor="end" fill="var(--sea-ink-soft)" fontSize="15" fontWeight="bold">8</text>
        <text x="125" y="55" textAnchor="middle" fill="var(--term-x)" fontSize="17" fontWeight="bold" fontFamily="serif" fontStyle="italic">{t(minorTerm)}</text>
        <text x="125" y="205" textAnchor="middle" fill="var(--term-x)" fontSize="17" fontWeight="bold" fontFamily="serif" fontStyle="italic">{t(minorTerm)}'</text>
        <text x="35" y="135" textAnchor="middle" fill="var(--term-y)" fontSize="17" fontWeight="bold" fontFamily="serif" fontStyle="italic" transform="rotate(-90 35 135)">{t(majorTerm)}</text>
        <text x="215" y="135" textAnchor="middle" fill="var(--term-y)" fontSize="17" fontWeight="bold" fontFamily="serif" fontStyle="italic" transform="rotate(-90 215 135)">{t(majorTerm)}'</text>
        {counters}
